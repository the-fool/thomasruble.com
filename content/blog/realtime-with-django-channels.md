...
I was recently charged to write a web-server that handles requests from its web-client for starting a long-running task on _another_ server, and then sending the results back once it's complete.  The web-server is something of a proxy, brokering events between a backend service and a web-client.  The backend service provides a way to retrive the progress of a task its working on, and so I wanted to update the client at regular intervals with the status of the task.
